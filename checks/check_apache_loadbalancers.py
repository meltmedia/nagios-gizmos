...
class ApacheLoadBalancer(nagiosplugin.Resource):
        ...
        for tag in soup.find_all("h1"):
            if "Proxy LoadBalancer Status" in tag.string:
                if self.balancer is None or self.balancer in tag.string:
                    ...
                    name = tag.string[
                        tag.string.index("balancer://") + len("balancer://"):]

                    # find all td's in the 2nd table with the text == Ok
                    alive_members = tag.find_next_sibling("table").\
                        find_next_sibling("table").find_all("td", text="Ok")
        ...
        if len(balancers) > 0:
            for balancer in balancers:
                yield nagiosplugin.Metric(
                    balancer[0], balancer[1], min=0, context='members')
        else:
            yield nagiosplugin.Metric(
                'balancers', int(-1), min=0, context='members')
    ...
    def main():
        ...
        argp.add_argument(
            '-b', '--balancer', action='store', default=None,
            help='which balancer to check')
        argp.add_argument(
            'url', help='apache server status url')
